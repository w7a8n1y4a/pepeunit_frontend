...
import { useGetUnitsOutputByInputLazyQuery, useDeleteUnitNodeEdgeMutation, PermissionEntities, useGetResourceAgentsLazyQuery } from '@rootTypes/compositionFunctions';
...
export default function PermissionForm({ currentNodeData }: PermissionFormProps) {
    ...
    const handleGetResources = (permissionEntities: PermissionEntities) => {
        ...
        if (currentNodeData) {
            getResourceAgentsLazyQuery({
                variables: {
                    agentType: permissionEntities,
                    resourceUuid: currentNodeData.uuid,
                    resourceType: PermissionEntities.Repo,
                }
            }).then(result => {
                if (result.data) {
                    setIsLoaderActive(false);
                    fetchNodeOutputs();
                    console.log(result.data.getResourceAgents)
                }
            }).catch(error => {
                setIsLoaderActive(false)
                setResultData({ type: ResultType.Angry, message: error.graphQLErrors[0].message.slice(4)})
